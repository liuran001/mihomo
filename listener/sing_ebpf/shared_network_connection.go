...
func (s *sharedNetwork) NewPacket(data []byte, oob []byte, source netip.AddrPort) {
	...
	if backend.UDPAssignmentEnabled() && hasPacketDestination &&
		...
		s.forwardUDPPacket(data, client, original.Destination, false)
		return
	}
	...
	s.forwardUDPPacket(data, client, cachedOriginal.Destination, cachedOriginal.ConnectedUDP)
}

func (s *sharedNetwork) forwardUDPPacket(data []byte, client netip.AddrPort, destination netip.AddrPort, connected bool) {
	metadata := &C.Metadata{
		NetWork: C.UDP,
		Type:    C.EBPF,
