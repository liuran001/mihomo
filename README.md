...
本分支给策略组加了两个选项，按**主动探测的结果**择优：UDP 用 STUN binding request 探回包，IPv6 用一个 v6-only 端点探连通性。探测走独立的状态检查，不写入节点的存活标记与延迟历史，所以面板上的健康状态仍然只反映真实的健康检查结果。

```yaml
proxy-groups:
  - name: 自动选择
    type: url-test          # 对 smart / url-test / load-balance 生效
    use: [机场A, 机场B]
    prefer-udp: true        # 优先支持 UDP 转发的节点
    prefer-ipv6: true       # 优先能走 IPv6 出站的节点
```

**是降优先级而不是过滤**。探测结果不会把节点从候选池里拿掉，只在排序时给它加一笔「惩罚延迟」：
...
两项偏好同时开启时惩罚累加，最多 1200 ms。于是一个 100 ms 但不走 IPv6 的节点（计 700 ms）仍然赢过一个 900 ms 的 IPv6 节点，而在延迟相近时具备能力的节点总是优先。

**关键在于节点永远留在池子里。** 探测本身并不可靠——IPv6 探测目标只有在上游 IPv6 正常时才可达，失败率很容易上到六成——如果探测失败就把节点踢出候选，一个组会因为与「能不能转发流量」毫无关系的原因被削光，甚至把 `DIRECT` 一起踢掉，让本该直连的流量悄悄绕进代理。

惩罚只作用于**自行按延迟排序**的组：

| 组类型 | 偏好的作用方式 |
| --- | --- |
| `url-test` / `smart` | 计入排序用的延迟 |
| `load-balance` | 优先在满足偏好的节点间分配流量，这些节点全部不可用时才降级使用其余节点 |
| `select` / `fallback` | 不介入 |

`select` 与 `fallback` 刻意不受影响：手动选定的节点和显式配置的优先级顺序，不该被一次探测结果改写。

探测结果按节点身份（名字 + 类型 + 提供者 + 地址）全局缓存，成功 30 分钟、失败 10 分钟，异步刷新，最多 8 个并发；并发占满时本轮直接跳过、留待下次重试，不排队堆积。**缓存在内存中，核心重启后需要重新探测**，收敛期间所有节点按「尚未探测」对待，只带最小惩罚。
